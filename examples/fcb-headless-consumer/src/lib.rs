...
use fcb::{BrowserSession, FcbError, Feature, FeatureSet, HostRequest, HostServices, MemorySourceProvider, SourceCapture};
use fcb_core::{
...
use fcb_search::{DirectSourceScanner, QueryOptions, SearchMode, SearchResult, UnicodeNormalization};
...
fn inert_startup_and_unsupported_feature_errors_keep_host_in_control() {
    ...
    let host = Arc::new(HeadlessHostServices::new(FeatureSet::all_known()));
    ...
    assert_eq!(host.redraw_requests.load(Ordering::SeqCst), 1);

    session_with_host
        .request_host(HostRequest::Wake)
        .expect("wake request forwards to host");
    assert_eq!(host.wake_requests.load(Ordering::SeqCst), 1);
...
fn two_isolated_browser_sessions_preserve_cache_privacy_and_independent_lifecycle() {
    ...
    assert_eq!(plan_b.bytes().len().get(), 17);
...
fn no_storage_in_memory_path_search_and_refinement() {
    ...
    let gen1 = QueryGeneration::new(owner, 1).unwrap();
    let mut exact_opts = PathSearchOptions::new(gen1);
    ...
    let gen2 = QueryGeneration::new(owner, 2).unwrap();
    let mut prefix_opts = PathSearchOptions::new(gen2);
    ...
    let gen3 = QueryGeneration::new(owner, 3).unwrap();
    let fuzzy_opts1 = PathSearchOptions::new(gen3);
    ...
    let gen4 = QueryGeneration::new(owner, 4).unwrap();
    let fuzzy_opts2 = PathSearchOptions::new(gen4);
...
fn negative_control_path_search_empty_query_and_foreign_owner() {
    ...
    let gen = QueryGeneration::new(owner, 1).unwrap();
    let empty_err = PathSearch::new(
        &index,
        b"",
        PathSearchOptions::new(gen),
        &budget,
