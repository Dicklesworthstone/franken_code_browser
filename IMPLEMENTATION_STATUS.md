...
| `crates/fcb-map` — partition layout | Deterministic retained partition tree: parent-local rectangles, frozen raw-path child order (not size-sorted squarify), capped-log byte weights, unknown placeholders, reserved slack, committed `LayoutRevision` restore. Evidence: `crates/fcb-map/tests/partition_layout.rs`, 10/10 via strict remote `cargo test -p fcb-map --test partition_layout -j 3` on hz2 (bead `fcb-d6x.1`, code-first, batch verification pending; claim blocked on open `fcb-hh2.2`). Does not implement local repair/repack (FCB-013.B) or pixels. |
