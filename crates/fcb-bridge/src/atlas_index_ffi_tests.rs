...
#[test]
fn all_four_index_entrypoint_types_and_header_declarations_agree() {
    ...
    for name in ["fcb_atlas_index_prepare(", "fcb_atlas_index_info(",
        "fcb_atlas_index_clear(", "fcb_atlas_search_indexed("] {
...
fn unknown_index_handles_return_bounded_independently_owned_error_strings() {
    // No global handle capacity is consumed by these contract tests. Actual
    // successful capture/query/activation uses independent real registry tests.
...
fn null_and_valid_foreign_queries_on_unknown_handles_never_produce_success_or_source_payloads() {
    let error = take(unsafe { fcb_atlas_search_indexed(u64::MAX, 1, 1, std::ptr::null(), 10, 1000) });
    assert!(error.contains("\"status\":\"error\""));
    let query = CString::new("private fixture phrase").unwrap();
    let error = take(unsafe { fcb_atlas_search_indexed(u64::MAX, 1, 1, query.as_ptr(), 10, 1000) });
    assert!(error.contains("\"status\":\"error\""));
