...
@MainActor final class AtlasMetalGlyphRenderer {
    struct Line {
    ...
    private let tileMemoryPipeline: MTLRenderPipelineState?
    private let tileMemoryAdditivePipeline: MTLRenderPipelineState?
    private let tileMemoryResolvePipeline: MTLRenderPipelineState?
    /// Below this source density, repeated 8-bit blend rounding loses small ink contributions.
    static let directBlendMinimumDensity = 0.1
    static let tileMemoryMinimumDensity = 0.04
    ...
    init(device: MTLDevice, tiles: [SceneTile], pixelsPerPoint: Double,
         ...
         maskPlacement: MaskPlacement = .displayGrid, tileMemoryAccumulation: Bool = false) throws {
        let rgbPhases = maskRepresentation == .opaqueRGBPhases
        let captureGrid = maskPlacement == .captureGrid
        guard !tileMemoryAccumulation || (captureGrid && !rgbPhases && opaqueBackgrounds && device.supportsFamily(.apple2)) else {
        ...
        do {
            ...
            if tileMemoryAccumulation {
                // Keep the precision buffer entirely in Apple GPU tile memory.
                // Only the final BGRA result is stored outside the render pass.
                attachment.pixelFormat = .rgba16Float
                attachment.writeMask = .all
                attachment.destinationRGBBlendFactor = .oneMinusSourceAlpha
                desc.colorAttachments[1].pixelFormat = .bgra8Unorm
                desc.colorAttachments[1].writeMask = []
                desc.fragmentFunction = library.makeFunction(name: "glyphFragment")
                tileMemoryPipeline = try device.makeRenderPipelineState(descriptor: desc)
                attachment.destinationRGBBlendFactor = .one
                attachment.writeMask = [.red, .green, .blue]
                desc.fragmentFunction = library.makeFunction(name: "glyphDeltaFragment")
                tileMemoryAdditivePipeline = try device.makeRenderPipelineState(descriptor: desc)
                ...
                resolve.colorAttachments[0].pixelFormat = .rgba16Float
                resolve.colorAttachments[0].writeMask = []
                resolve.colorAttachments[1].pixelFormat = .bgra8Unorm
                tileMemoryResolvePipeline = try device.makeRenderPipelineState(descriptor: resolve)
            } else {
                tileMemoryPipeline = nil; tileMemoryAdditivePipeline = nil; tileMemoryResolvePipeline = nil
            }
    ...
    func encode(into target: MTLTexture, commandBuffer: MTLCommandBuffer,
                scale: Double, offset: CGPoint, tileIDs: [Int]? = nil,
                raster: AtlasMetalRasterRenderer? = nil, rasterIDs: [Int] = []) throws {
        ...
        if let raster { try raster.validate(scale: scale, tileIDs: rasterIDs, deviceRegistryID: device.registryID) }
        else if !rasterIDs.isEmpty { throw Fallback.invalidGeometry }
        ...
        let useTileMemory = tileMemoryPipeline != nil && selected.contains { id in
            scale * pixelsPerPoint / batches[id]!.pixelsPerWorldPoint < Self.directBlendMinimumDensity
        }
        let accumulation: MTLTexture?
        if useTileMemory {
            // The presenter bounds outstanding commands. This texture has no
            // external pixel allocation and is retained by its command buffer.
            guard target.width <= 32 * 1024 * 1024 / max(1, target.height) else { throw Fallback.resourceLimit }
            ...
            guard let texture = device.makeTexture(descriptor: descriptor) else { throw Fallback.allocation }
            accumulation = texture
        } else { accumulation = nil }
        let pass = MTLRenderPassDescriptor()
        pass.colorAttachments[0].texture = accumulation ?? target
        pass.colorAttachments[0].loadAction = .clear; pass.colorAttachments[0].storeAction = useTileMemory ? .dontCare : .store
        pass.colorAttachments[0].clearColor = MTLClearColorMake(0, 0, 0, 0)
        if useTileMemory {
            pass.colorAttachments[1].texture = target
            pass.colorAttachments[1].loadAction = .dontCare
            pass.colorAttachments[1].storeAction = .store
        }
        let backgroundPipeline = useTileMemory ? tileMemoryPipeline! : pipeline
        let inkPipeline = useTileMemory ? tileMemoryAdditivePipeline! : additivePipeline
        ...
        // CPU overview layers formerly composited above Metal glyphs. Preserve
        // that order even where adjacent parcels share a fractional edge pixel.
        raster?.encode(encoder: encoder, tileMemory: useTileMemory, width: target.width, height: target.height,
            scale: scale, offset: offset, tileIDs: rasterIDs)
        if useTileMemory {
            encoder.setRenderPipelineState(tileMemoryResolvePipeline!)
            encoder.drawPrimitives(type: .triangleStrip, vertexStart: 0, vertexCount: 4)
        }
        encoder.endEncoding()
    }
    ...
    vertex float4 resolveVertex(uint i [[vertex_id]]) {
        const float2 corners[4] = {float2(-1,-1),float2(1,-1),float2(-1,1),float2(1,1)};
        return float4(corners[i],0,1);
    }
    struct ResolvedPixel { float4 value [[color(1)]]; };
    fragment ResolvedPixel resolveFragment(float4 accumulated [[color(0)]]) {
        return ResolvedPixel{accumulated};
    }
    ...
    vertex Out glyphVertex(uint vertexIndex [[vertex_id]], uint instance [[instance_id]],
        ...
        // Same two triangles, sharing their diagonal: four vertex invocations per instance.
